"""
model_engine.py  —  Regional Economic Impact Model  v6.0
─────────────────────────────────────────────────────────
Pure-computation module. No Streamlit imports. No side effects.

Changes from v5:
  1. Sector 14 split into three — Private Other Services (14),
     Federal Gov & Defense (15), State & Local Gov (16)
     N grows from 15 → 17. All crosswalks, fallbacks, and QCEW
     parsing updated to reflect the split.
  2. Uncertainty bands on all outputs (±25% methodological floor,
     ±15% data uncertainty, combined ±30% reported)
  3. Sensitivity analysis — compute impacts across FLQ delta range
  4. Multi-scenario store — run and compare multiple projects
  5. All v5 improvements preserved
"""
...
# Sector colours for charts (stable across runs)
SECTOR_COLORS = [
...
# Uncertainty parameters (documented in Watson et al. 2015, Loomis & White 1996)
# Methodological uncertainty: ±25% (I-O model vs observed outcomes)
# Data vintage uncertainty:   ±15% (2017 tables applied to current economy)
# Combined (root-sum-square): ±30%
UNCERTAINTY_PCT = 0.30

# ── SECTOR CROSSWALK ──────────────────────────────────────────────────────────
# BEA special codes updated: GSLGE/GSLGH/GSLGO → 16 (was 14)
# Federal BEA codes → 15; State/local BEA codes → 16

_SPECIAL = {
    # Scrap, used goods, adjustments → private other (14)
    "S00500":14,"S00600":14,"S00101":14,"S00102":14,
    "S00201":14,"S00202":14,"S00203":14,
    # State/local govt → 16
    "GSLGE":16,"GSLGH":16,"GSLGO":16,
    # Federal govt & defense → 15
    "GFGD":15,"GFGN":15,"GFE":15,
    # State & local govt (BEA summary codes) → 16
    "GSLE":16,"GSLO":16,
    # Exclude: non-comparable imports, rest-of-world
    "S00401":-1,"S00402":-1,"S00300":-1,"S00900":-1,
    # Real estate sub-codes → 9
...
NAICS_TO_SECTOR = {
    ...
    # Arts & Accommodation
    "71":13,"72":13,
    # Private Other Services (NAICS 81)
    "81":14,"811":14,"812":14,"813":14,"814":14,
    # Government — split by sub-code
    "921":15,                          # Federal (exec, legislative, judicial)
    "922":16,"923":16,"924":16,        # State/local justice, admin, finance
    "925":16,"926":16,"927":16,"928":16,
    "92":16,                           # Generic "92" defaults to state/local
                                       # (most county govt is state/local)
}

# BEA industry-code → sector mapping (updated for split)
BEA_SUMMARY_TO_SECTOR = {
    ...
    "81":14,                       # Private other services
    "GFGD":15,"GFGN":15,"GFE":15, # Federal gov & defense
    "GSLE":16,"GSLO":16,           # State & local gov
}
...
def _code_to_sector(code: str) -> int:
    ...
    if   p2 == "11":             return 0
        ...
        # Try 3-digit for federal vs state/local
        if p3 == "921":          return 15
        else:                    return 16
    else:                        return 14
...
def load_qcew(src) -> dict:
    """
    Parse BLS QCEW county CSV.
    Sector 14 split: private NAICS 81 → 14, federal → 15, state/local → 16.
    """
    df = pd.read_csv(src)
    ...
    # Manufacturing durable/non-durable split
    ...
    # ── Government split: own_code=1 → federal (15), 2+3 → state/local (16)
    # Federal government (own_code=1)
    fed_rows = df[(df["own_code"]==1) & (df["agglvl_code"]==71)]
    if len(fed_rows):
        fed_emp  = float(fed_rows["annual_avg_emplvl"].sum())
        fed_wage = float(fed_rows["total_annual_wages"].sum())
        emp[15]    += fed_emp
        wage_n[15] += fed_wage
        lq_n[15]   += fed_rows.apply(
            lambda r: r["lq_annual_avg_emplvl"] * r["annual_avg_emplvl"], axis=1
        ).sum()

    # State government (own_code=2)
    state_rows = df[(df["own_code"]==2) & (df["agglvl_code"]==71)]
    if len(state_rows):
        state_emp  = float(state_rows["annual_avg_emplvl"].sum())
        state_wage = float(state_rows["total_annual_wages"].sum())
        emp[16]    += state_emp
        wage_n[16] += state_wage
        lq_n[16]   += state_rows.apply(
            lambda r: r["lq_annual_avg_emplvl"] * r["annual_avg_emplvl"], axis=1
        ).sum()

    # Local government (own_code=3)
    local_rows = df[(df["own_code"]==3) & (df["agglvl_code"]==71)]
    if len(local_rows):
        local_emp  = float(local_rows["annual_avg_emplvl"].sum())
        local_wage = float(local_rows["total_annual_wages"].sum())
        emp[16]    += local_emp
        wage_n[16] += local_wage
...
def compute_flq_rpc(lq, total_county_emp, total_national_emp, delta=0.25):
...
def regionalize(A_nat17, qcew, national_emp_total, delta=0.25) -> dict:
    ...
    A_york = A_nat17 * rpc_york[:, np.newaxis]
    return {
        "A_york":  A_york, "lq_york": lq_york,
        "rpc_york": rpc_york, "rpc_slq": rpc_slq,
        "lambda": lam, "delta": delta,
    }

# ── BLS / BEA API & FALLBACKS ─────────────────────────────────────────────────

# Original 14 sectors + private other services (unchanged key=14)
# Plus three new series for government split
BLS_SERIES = {
    ...
    14:   "CEU8000000001",   # Private other services only
    15:   "CEU9091000001",   # Federal government (NEW)
    "16a":"CEU9092000001",   # State government   (NEW)
    "16b":"CEU9093000001",   # Local government   (NEW)
}
...
BLS_FALLBACK_2022 = {
    ...
    "CEU9091000001":2997.0,   # Federal gov 2022
    "CEU9092000001":5220.0,   # State gov 2022
    "CEU9093000001":14307.2,  # Local gov 2022
}

# 17-element fallback arrays (sectors 0-16)
...
def fetch_bea_table(api_key, table_id, year="2022"):
    params = {"UserID":api_key,"method":"GetData","datasetname":"GDPbyIndustry",
              "TableID":table_id,"Frequency":"A","Year":year,
...
def series_to_emp(raw) -> np.ndarray:
    ...
    emp[15] = raw.get(BLS_SERIES[15],  0.0)           # Federal gov
    emp[16] = raw.get(BLS_SERIES["16a"],0) + raw.get(BLS_SERIES["16b"],0)  # State+local
    return emp


def _use_or_fallback(result, api_fallback, csv_filename, min_plausible_sum=None):
...
def build_employment_coefficients(national_emp, bea_output, bea_comp,
    ...
    return {
        ...
        "jobs_per_million":   jobs_per_va,  # alias
        "avg_wage":           york_wage,
...
def compute_impacts(Y, inverses, emp_coeffs) -> dict:
    ...
    return {
        ...
        "vecs":{"direct":direct,"indirect":indirect,"induced":induced,"total":total},
        "diagnostics":{
...
def add_uncertainty(results, pct=UNCERTAINTY_PCT) -> dict:
    """
    Add ±pct uncertainty bands to all scalar outputs.

    Basis:
      Watson et al. (2015) — I-O model vs observed outcomes: ±20–30%
      2017 vintage tables applied to 2024 economy: additional ±10–15%
      Combined (conservative): ±30%

    Returns results dict with 'bands' sub-dict added to each impact type.
    """
    out = {}
...
def sensitivity_analysis(
    Y, A_nat17, qcew, national_emp_total,
    emp_coeffs, pce_shares,
    delta_range=(0.10, 0.15, 0.20, 0.25, 0.30, 0.35),
) -> list:
    """
    Run impacts across a range of FLQ delta values.
    Returns list of dicts: {delta, lambda, total_jobs, total_output, type2_mult}
    """
    rows = []
    for delta in delta_range:
        reg = regionalize(A_nat17, qcew, national_emp_total, delta=delta)
        ...
        rows.append({
            ...
            "avg_rpc_flq": round(float(reg["rpc_york"].mean()), 4),
        })
    return rows
...
def validate_coefficients(emp_coeffs) -> list:
    ...
    for s in range(N):
        ...
        flags = []
        if not (0.5 <= jpm <= 40.0):
            flags.append(f"jobs/$1M VA={jpm:.2f} out of [0.5–40]")
        if not (15_000 <= wage <= 200_000):
            flags.append(f"wage=${wage:,.0f} out of range")
        if not (0.05 <= li <= 0.70):
            flags.append(f"LI%={li:.1%} out of [5–70%]")
...
def make_scenario(name, naics, investment, results_with_bands, run_params) -> dict:
    """
    Package a completed run into a storable scenario dict.
    The scenario store in app.py is a list of these.
    """
...
def build_model(use_file, ms_file, dom_file, qcew_file,
                ...
                flq_delta=0.25, mpc=0.90, regional_retention=0.50,
    ...
    bea_output, bea_output_src = _use_or_fallback(
        fetch_bea_table(bea_api_key,"5"), BEA_OUTPUT_FALLBACK,
        "bea_output.csv", min_plausible_sum=10_000_000)
    bea_comp, bea_comp_src = _use_or_fallback(
        fetch_bea_table(bea_api_key,"2"), BEA_COMP_FALLBACK,
        "bea_compensation.csv", min_plausible_sum=3_000_000)
    bea_va, bea_va_src = _use_or_fallback(
        fetch_bea_table(bea_api_key,"1"), BEA_VA_FALLBACK,
    ...
    return {
        "A_nat17":    A_nat17,  "A_york":     reg["A_york"],
        "lq_york":    reg["lq_york"],
        "rpc_york":   reg["rpc_york"], "rpc_slq": reg["rpc_slq"],
        "flq_lambda": reg["lambda"],   "flq_delta": flq_delta,
        "pce_shares": pce_shares,
        "inverses":   inverses,
        "emp_coeffs": emp_coeffs,
        "national_emp": national_emp,
        "bea_output": bea_output, "bea_comp": bea_comp, "bea_va": bea_va,
        "qcew":       qcew,
        ...
        "import_leakage_rate":     float(1.0 - reg["rpc_york"].mean()),
        ...
        "A_nat15":    A_nat17,   # alias for backwards compat
        "scenarios":  [],        # filled by app.py
    }
