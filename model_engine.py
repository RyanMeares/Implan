...
def _use_or_fallback(result_and_diag, api_fallback, csv_filename,
    ...
    """
    Three-tier fallback. Accepts either a bare array or a (array, diag) tuple
    as returned by fetch_bea_table.
    Returns (array, source_label, diagnostic_string).
    """
    # Unpack tuple if fetch_bea_table returned (result, diag)
    ...
    if result is not None and result.sum() > 0:
        if min_plausible_sum is None or result.sum() >= min_plausible_sum:
            return result, "live", diag
        else:
            return_diag = (f"FALLBACK — plausibility check failed "
                           f"(sum=${result.sum()/1e6:.1f}T < "
                           f"threshold ${min_plausible_sum/1e6:.0f}T). {diag}")
            csv_result = _load_csv_cache(csv_filename)
            if csv_result is not None:
                return csv_result, "csv_cache", return_diag
            return api_fallback, "hardcoded_fallback", return_diag

    return_diag = f"FALLBACK — API returned no data. {diag}"
    csv_result = _load_csv_cache(csv_filename)
    if csv_result is not None:
        return csv_result, "csv_cache", return_diag
    return api_fallback, "hardcoded_fallback", return_diag
