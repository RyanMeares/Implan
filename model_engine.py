...
def load_qcew(src: Any) -> dict:
    ...
    if len(mfg3) > 0:
        ...
        # Reset both mfg sectors before re-accumulating from 3-digit rows
        for s_idx in (3, 4):
            emp[s_idx] = lq_n[s_idx] = wage_n[s_idx] = 0.0
    ...
    # Wage from total_annual_wages for government; avg_annual_pay for private
    # Impute national average wage for sectors with zero local employment
...
def build_pce_shares(pce_raw: np.ndarray, com_sector: np.ndarray) -> np.ndarray:
    """
    Aggregate PCE final demand across 402 commodities to 17 sector shares.

    Returns a normalised share vector summing to 1.0. Falls back to a
    uniform distribution if all PCE values are zero.

    Parameters
    ----------
    pce_raw    : (402,) PCE final demand by commodity ($M, from Use Table)
    com_sector : (402,) sector index for each commodity

    Returns
    -------
    np.ndarray (17,) — share of household spending directed to each sector
    """
