"""
Regional Economic Impact Analysis Tool  v7.0
...
<div class="app-header">
  ...
  <p>v7.0 · Supply-Demand Pool Regionalization · 17 Sectors · Uncertainty Bands · Sensitivity · Scenario Comparison</p>
</div>
""", unsafe_allow_html=True)
    ...
    if st.button("🔨 Build Model", disabled=not all_up,
        ...
        with st.spinner("Building…"):
            try:
                ...
                # Shows immediately after build so you always know
                # whether live API data or fallback values were used.
                src_map = {
                    "live":                ("LIVE", "✅"),
                    "csv_cache":           ("CSV fallback", "📁"),
                ...
                # comp is derived from VA so inherits its source
                comp_lbl = va_lbl + " (derived)"

                all_live = (state["bea_va_src"] == "live" and
                            state["bea_output_src"] == "live")

                if all_live:
                    st.success("✅ All BEA data loaded from live API")
                else:
                    st.warning("⚠️ One or more BEA tables used fallback data")
...
| QCEW County | ✅ FILE | {state['qcew']['emp'].sum():,.0f} workers, FIPS {state['qcew']['fips']} {state['qcew']['year']} |
| BEA I-O Structure | ✅ FILE | 402-industry BEA benchmark tables |
                """)

                # Show API diagnostic detail when fallback triggered
                if not all_live:
...
with tab_res:
    ...
    if sj:
        lbls,vls=zip(*sj[:10])
        clrs5=[me.SECTOR_COLORS[me.SECTOR_LABELS.index(l)] for l in lbls]
...
with tab_scen:
    ...
    st.caption(f"{len(scens)} scenario(s). Run more from the Run tab to compare.")
    comp_rows=[{
...
with tab_sens:
    ...
    # Chart — SDP alpha sweep only (exclude SLQ/FLQ from line chart)
    ...
    for ax,(ykey,title,ylabel) in zip(axes_s,[
        ...
        # Reference lines for SLQ and FLQ
        if slq_row:
...
with tab_sec:
    ...
    st.markdown('<div class="section-header">17-Sector Output ($)</div>',unsafe_allow_html=True)
    out_rows=[]
    for s in range(me.N):
...
with tab_coef:
    ...
    if any(not v["ok"] for v in val_results):
        st.warning("⚠️ One or more coefficients outside plausible ranges.")
    else:
        st.success("✅ All 17 employment coefficients within plausible ranges.")
    coeff_rows=[{
    ...
    qr=[{"Sector":me.SECTOR_LABELS[s],
         ...
         "Employment":f"{ms['qcew']['emp'][s]:,.0f}","LQ":f"{ms['qcew']['lq'][s]:.3f}",
         "RPC (SDP)":f"{ms['rpc_sdp'][s]:.4f}","Avg Annual Pay":fmt(ms['qcew']['wage'][s])}
        for s in range(me.N)]
...
with tab_diag:
    ...
    | Parameter | Value |
    |---|---|
    | Sectors | 17 (gov split: Private / Federal / State+Local) |
...
with tab_exp:
    ...
    with c1:
        ...
        st.download_button("📥 Impact Summary CSV",
            ...
            file_name="impact_summary_v7.csv",mime="text/csv",use_container_width=True)
        sec_rows=[]
        for s in range(me.N):
            dj=res["direct"]["jobs_vec"][s]; ij=res["indirect"]["jobs_vec"][s]; nj=res["induced"]["jobs_vec"][s]
            sec_rows.append({
        ...
        st.download_button("📥 Sector Detail CSV",
            ...
            file_name="impact_by_sector_v7.csv",mime="text/csv",use_container_width=True)
    with c2:
        m_=res["multipliers"]
        mult_df=pd.DataFrame([{
            "Project":rp["project_name"],"NAICS":rp["naics"],
            "Region":f"FIPS {ms['qcew']['fips']}","Investment":rp["investment"],
            "Sectors":17,"IO_Data":"BEA 2017 Benchmark After-Redefinitions Domestic",
        ...
        st.download_button("📥 Multipliers CSV",
            ...
            file_name="multipliers_v7.csv",mime="text/csv",use_container_width=True)
        if st.session_state.scenarios:
            ...
            st.download_button("📥 Scenario Comparison CSV",
                ...
                file_name="scenario_comparison_v7.csv",mime="text/csv",use_container_width=True)
        ...
        st.download_button("📥 A_york Matrix CSV",
            ...
            file_name="A_york_SDP_17sector.csv",mime="text/csv",use_container_width=True)
    st.markdown("---")
    st.markdown(f"""
**v7.0:** 17-sector · SDP regionalization α={ms['sdp_alpha']:.2f} ·
