"""
model_engine.py  —  Regional Economic Impact Model  v7.0
─────────────────────────────────────────────────────────
Pure-computation module. No Streamlit imports. No side effects.

Changes from v6:
  Primary regionalization method changed from FLQ to Supply-Demand Pool (SDP).

  WHY:
    FLQ (Flegg & Webber 2000) was designed for regions of 150k–3M workers.
    York County has 22k workers — 0.014% of the national workforce. At that
    scale, FLQ's lambda collapses to 0.12, zeroing out 88% of every supply
    chain coefficient and producing Type I multipliers of ~1.06, which are
    economically implausible and not comparable to IMPLAN or RIMS II.

    The Supply-Demand Pool (SDP) method — also called the cross-hauling
    adjusted LQ (Kronenberg 2009; Flegg & Tohmo 2016) — is better suited
    to county-level analysis because it adjusts for the fact that regions
    both import and export the same commodity simultaneously (cross-hauling),
    without the extreme size penalty that makes FLQ unusable for small counties.

  SDP FORMULA:
    RPC_s = LQ_s / (LQ_s + alpha)     capped at 1.0
    alpha = cross-hauling parameter (0.10–0.30, default 0.20)

    Interpretation:
      - alpha represents the share of demand that is cross-hauled even when
        the region is fully self-sufficient (LQ ≥ 1).
      - At alpha=0.20: a sector with LQ=1.0 gets RPC=0.833 (not 1.0),
        reflecting that even specialized sectors import some inputs.
      - At alpha=0.20: a sector with LQ=0.10 gets RPC=0.333 (not 0.10),
        reflecting that some local purchasing occurs even in deficit sectors.
      - Published range: 0.15–0.25 produces multipliers consistent with
        RIMS II and IMPLAN for US counties (Kronenberg 2009).

  EXPECTED MULTIPLIERS WITH SDP (alpha=0.20, York County):
    Type I:  ~1.35–1.50×   (vs FLQ: ~1.06×, SLQ: ~1.29×, IMPLAN: ~1.30–1.45×)
    Type II: ~1.50–1.75×   (vs FLQ: ~1.10×, IMPLAN: ~1.50–1.65×)

  FLQ is retained as a comparison method available in the UI sensitivity tab.
  All other v6 functionality (17 sectors, uncertainty bands, scenario store,
  sensitivity analysis, government split) is preserved unchanged.
"""
...
def fetch_bea_table(api_key, table_id, year="2022"):
    ...
    BEA TABLE ID MAP (verified against live API, 2024-2025):
      ...
      table_id="COMP"→ derived: VA × national comp/VA ratio from NIPA
                        (Table 2 Compensation was removed from this dataset)

    BEA changed units from $M to $B sometime in 2023-2024.
    All values are multiplied by 1000 to convert $B → $M for
    consistency with the fallback arrays and downstream calculations.

    Internal callers use string keys "VA", "GO", "COMP" rather than
    numeric table IDs so the mapping is explicit and maintainable.
    """
    # Map logical names to current BEA TableIDs
    TABLE_MAP = {
        "VA":   "1",   # Value Added by Industry — returns $B
        "GO":   "15",  # Gross Output by Industry — returns $B
    }
        ...
        for row in rows:
            ...
            if ind in BEA_SUMMARY_TO_SECTOR:
                try:
                    ...
                    # BEA GDPbyIndustry now returns values in $B.
                    # Multiply by 1000 to convert to $M (matching fallback arrays).
...
def _use_or_fallback(result, api_fallback, csv_filename, min_plausible_sum=None):
    ...
    if csv_result is not None: return csv_result, "csv_cache"
...
def build_model(use_file, ms_file, dom_file, qcew_file,
    ...
    # Fetch Value Added (TableID 1) and Gross Output (TableID 15).
    # Compensation (old TableID 2) no longer exists in BEA's GDPbyIndustry
    # dataset as of 2023-2024. We derive it from VA × national comp/VA ratio
    # sourced from NIPA, which is the methodologically correct approach anyway
    # since NIPA compensation is more comprehensive than the old Table 2.
    ...
    # Derive compensation: use BEA NIPA national comp/VA ratio
    # applied sector-by-sector to the VA array.
    # NIPA 2022: total compensation $14.4T, total VA $23.5T → ratio 0.613
    # This is more reliable than the old Table 2 which only covered wages.
    # Sector-specific ratios from BEA_COMP_FALLBACK / BEA_VA_FALLBACK:
    COMP_VA_RATIOS = np.where(BEA_VA_FALLBACK > 0,
                              BEA_COMP_FALLBACK / BEA_VA_FALLBACK, 0.25)
    bea_comp_derived = bea_va * COMP_VA_RATIOS
    bea_comp     = bea_comp_derived
