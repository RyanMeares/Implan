...
with tab_mixed:
    ...
    if st.button("▶  Run Mixed-Use Analysis",
        ...
        with st.spinner("Running component analyses…"):
            ...
            # Save to scenario store as a pre-built record to bypass naics_to_sector
            total_spend = sum(c["spend"] for c in components if c["valid"])
            scens = st.session_state.scenarios or []
            t_ = res_combined["total"]
            m_ = res_combined["multipliers"]
            scens.append({
                "name":         mu_name + " (Mixed-Use)",
                "naics":        "multi",
                "investment":   float(total_spend),
                "sector":       "Mixed-Use Development",
                "output":       t_["output"],
                "jobs":         t_["jobs"],
                "jobs_low":     t_["bands"]["jobs_low"],
                "jobs_high":    t_["bands"]["jobs_high"],
                "labor_income": t_["labor_income"],
                "value_added":  t_["value_added"],
                "type1":        m_["type1"],
                "type2":        m_["type2"],
                "emp_mult":     m_["emp"],
                "params":       {"components": [c["name"] for c in components]},
                "full_results": res_combined,
            })
            st.session_state.scenarios = scens
...
with tab_res:
    ...
    st.dataframe(pd.DataFrame(impact_rows),hide_index=True,width='stretch')
...
with tab_sec:
    ...
    st.dataframe(pd.DataFrame(job_rows),hide_index=True,width='stretch')
    ...
    st.dataframe(pd.DataFrame(lq_rows),hide_index=True,width='stretch')
...
with tab_coef:
    ...
    st.dataframe(pd.DataFrame(coeff_rows),hide_index=True,width='stretch')
    ...
    qr=[{"Sector":me.SECTOR_LABELS[s],
        ...
        for s in range(me.N)]
    st.dataframe(pd.DataFrame(qr),hide_index=True,width='stretch')
    st.caption(
